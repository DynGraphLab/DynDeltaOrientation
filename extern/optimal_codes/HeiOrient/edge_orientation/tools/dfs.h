...
improve_paths_stack(
  ...
  while (stack_pointer >= 0) {
    ...
    if (index == 0) {
      {
        for (size_t i = start_u; i < count_u; i++) {
          if (instance.outflow(instance.nodeInEdge1(instance.flat_edge_info[i])) < max_flow - 1) {
      ...
      for (size_t i = start_v; i < count_v; i++) {
        if (instance.outflow(instance.nodeInEdge0(instance.flat_edge_info[i])) < max_flow - 1) {
          instance.flip_edge(instance.flat_edge_info[i]);
          visited_array[v] = false;
          found = true;
          break;
    ...
    for (InsideArrayRefType i = start_u + index; i < count_u; i++) {
      ...
      if (visited_array[w] == 0 && instance.outflow(w) < max_flow) {
...
improve_paths_stack(
  ...
  while (stack_pointer >= 0) {
    ...
    if (index == instance._nodes_offset[v]) {
      for (auto i = instance._nodes_offset[v];
           i < instance._nodes_offset[v] + InsideArrayRefType{instance._nodes_out[v]}; i++) {
        if (instance.outflow(instance.flat_edge_info[i]) < max_flow - 1) {
          instance.flip_active_edge(v, i);
          visited_array[v] = false;
          found = true;
          break;
        }
...
template <class FlipGraph>
void solve_by_dfs(FlipGraph& instance) {
  using InsideArrayRefType = typename FlipGraph::InsideArrayRefType;
  using IndexType = typename FlipGraph::IndexType;
  TIMED_FUNC(tere);
