...
int main(int argn, char **argv)
{
        ...
        struct arg_lit *filter                      = arg_lit0(NULL, "filter","Insert edges only once.");
        struct arg_int *sliding_window              = arg_int0(NULL, "sliding_window", NULL, "Sliding window size m/x (x parameter).");
        struct arg_end *end                         = arg_end(100);

        // Define argtable.
        void* argtable[] = {
                filename, shuffle, user_seed, filter, sliding_window, user_delete_b, reinsert_deleted, help, end
        };
        // Parse arguments.
