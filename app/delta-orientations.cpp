...
int main(int argc, char **argv) {
  ...
  std::pair<int, int> p =
      graphIO.readEdgeSequence(graph_filename, edge_sequence);
  int n = p.first;
  ...
  std::shared_ptr<dyn_edge_orientation> algorithm =
      getdyn_edge_orientation_instance(config.algorithmType, G, config, res);
  ...
  if (config.num_queries) {
    ...
    std::cout << "time per query  \t" << fixed << setprecision(20)
              << elapsed_per_query << std::endl;

    srand(config.seed);
    random_functions::setSeed(config.seed);
    std::vector<std::pair<NodeID, NodeID>> queries_adjcent;
    for (unsigned i = 0; i < config.num_queries; i++) {
      queries_adjcent.push_back(
          edge_sequence[random_functions::nextInt(0, edge_sequence.size() - 1)]
              .second);
    }
    t.restart();
    ...
    for (unsigned i = 0; i < config.num_queries; i++) {
      num2 += algorithm->adjacent(queries_adjcent[i].first,
                                  queries_adjcent[i].second);
    }

    elapsed_per_query = t.elapsed() / config.num_queries;
    std::cout << "t*ime per adjqry \t" << fixed << setprecision(20)
              << elapsed_per_query << std::endl;
  ...
  if (config.output_filename.size() != 0) {
    std::cout << "writing orientation to " << config.output_filename
              << std::endl;
