#ifndef DELTA_ORIENTATIONS_MAXDECENDING_H
#define DELTA_ORIENTATIONS_MAXDECENDING_H
...
class dyn_edge_orientation_MaxDecending : public dyn_edge_orientation {
        public:
                dyn_edge_orientation_MaxDecending(const std::shared_ptr<dyn_graph_access>& GOrientation, const DeltaOrientationsConfig& config,
        ...
        private:
                ...
                int findsequence(NodeID source, int depth);
                void checkdegrees() {
                        int max = 0;
                        for( unsigned i = 0; i < GOrientation->number_of_nodes(); i++) {
