...
void dyn_edge_orientation_CCHHQRS::handleInsertion(NodeID source,
  ...
  // ignore existing edges (simple graphs only) that is the assumption.
  ...
  for (unsigned i = 0; i < config.b; i++) {
    if (vertices[source].out_degree <= vertices[target].out_degree) {
      insert_directed(new_uv, source);
    } else {
      insert_directed(new_vu, target);
    }
  }
}
...
void dyn_edge_orientation_CCHHQRS::handleDeletion(NodeID source,
  ...
  while (uv->count + vu->count > 0) { // TODO henrik question this
    if (uv->count >= vu->count) {
      delete_directed(uv, source);
    } else {
      delete_directed(vu, target);
    }
  }
}
...
void dyn_edge_orientation_CCHHQRS::add(DEdge *uv, NodeID u) {
  ...
  if (uv->count == 1) { // new edge
    ...
    vertices[uv->target].in_edges.add(uv, vertices[u].out_degree,
                                      vertices[uv->target].self_loop->bucket);
  }
}
...
void dyn_edge_orientation_CCHHQRS::remove(DEdge *uv, NodeID u) {
  ...
  if (uv->count == 0) {
    // uv->mirror->target->out_edges.erase(uv->location_out_neighbours);
