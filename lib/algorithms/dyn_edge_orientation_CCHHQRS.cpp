...
void dyn_edge_orientation_CCHHQRS::handleInsertion(NodeID source,
  ...
  if (edges.count(make_pair(source, target)) == 1) {
    return;
  }
  if (edges.count(make_pair(target, source)) == 1) {
    return;
  }
  // make the edges
  DEdge uv(u, v);
...
void dyn_edge_orientation_CCHHQRS::insert_directed(
  ...
  for (DEdge *uw : uv->source->out_edges) {
    ...
    if (uw->target->out_degree < uw_min->target->out_degree &&
        uw->target->node != uv->source->node) {
      uw_min = uw;
    }
  }

  if (uv->source->out_degree >
      std::max(config.b / 4, (1 + config.lambda) * uw_min->target->out_degree +
                                 config.theta)) {
    remove(uw_min);
...
void dyn_edge_orientation_CCHHQRS::delete_directed(
    DEdge *uv) { // NOLINT(*-no-recursion)
  remove(uv);
  DEdge *ux = uv->source->in_edges.buckets[uv->source->in_edges.max_bucketID]
                  .bucket_elements.front();
  if (ux->target->out_degree >
      max(config.b / 4,
          (1 + config.lambda) * ux->source->out_degree + config.theta)) {
    add(ux);
